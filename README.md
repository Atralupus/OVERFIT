...
> | 되는 것 | 아직 안 되는 것 |
> |---|---|
> | 대시 · 점프 · 패리 · 공격, 보스 패턴 3종(백장), 히트 판정 | **딥러닝 패턴 선택** — 이 저장소의 목적 그 자체 |
...
> [`overfit/data/credits.json`](overfit/data/credits.json) 에 한 줄이 늘고, 그 파일이 라이선스 주장의 전부다.
> 게임 안 크레딧 화면도 같은 파일을 읽는다.
...
| **그림** (스프라이트 · 배경) | 각 팩의 라이선스 — 지금은 넷 다 **CC0 1.0** |
...
같은 목록을 두 곳에 손으로 적으면 팩이 바뀌는 날 한쪽만 고쳐지고, 그 어긋남은 아무 테스트도 빨갛게 하지 않는다.
...
받는 법은 [`overfit/assets/LICENSES.md`](overfit/assets/LICENSES.md) 에 있다.
