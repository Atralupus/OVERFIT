...
| | 무엇을 막나 |
|---|---|
| **1단계 — 내려찍기 I** | 아무것도 안 막는다. 대시 · 가드 · 패리 · 간격이 전부 성립한다 — 당신이 무엇에 기대는지를 **재는** 단계다 |
| **II·III-끌기** | **대시.** 마무리를 밖으로 한 대시의 착지점에 세운다. 붙어 있으면 안 맞는다 |
| **II·III-쇄도** | **간격.** 마무리의 사거리가 도주로를 덮는다. 붙어 싸우면 아무 차이도 없다 |
| **II·III-쐐기** | **가드.** 넷째 타격이 세 배로 무거워 스태미나가 먼저 바닥난다. 패리는 여전히 싸다 |
...
**마무리(3타)만 다른 물건이다.** 예고가 빨갛고, 받아치면 보스가 **2.3초** 굳는다 — 최대 차지
한 방(2.0833초)이 그 안에 들어간다. "받아쳤다 → 제일 센 걸 꽂는다" 가 한 동작으로 이어지는 것이
이 게임의 고리이고, **1단계부터 그렇다.** 3단계의 마무리는 거기에 한 겹이 더 붙어 **가드로도
못 막는다**(危).
...
**색과 기호가 다른 말을 한다.** 마무리(3타) 앞에서 예고가 **호박에서 빨강으로** 바뀌고,
그중 가드로도 못 막는 것에만 **危** 한 글자가 더 뜬다. 빨강은 "받아칠 값이 있다" 까지고,
"막을 수조차 없다" 는 글자가 진다 — 붉은 것을 피하는 습관이 여기서는 정확히 거꾸로라,
**무엇을 하라(받아쳐라)는 말을 색에 맡길 수 없다.**

| <img src="docs/shots/battle-6-windup.png" width="420"> | <img src="docs/shots/battle-10c-guard-break-tell.png" width="420"> |
|---|---|
| 1·2타 — 호박. 흘려도 된다 | 마무리 — 빨강. 危 가 얹히면 가드도 안 된다 |
