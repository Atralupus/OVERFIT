...
> | 승패 · 결과 화면 · 단계 진행 | 캐릭터 3택 · 스탯 강화 (이슈 #22 로 뺐다 · 캐릭터는 [#38](https://github.com/Atralupus/OVERFIT/issues/38) 에서 하나로) |
...
| **대시** | Shift | 한 번에 367px 나간다. 보스 몸 하나를 확실히 지나는 거리이고, 그 동안 무적이다 |
| **패리** | F | 제자리에서 받아친다. 아래 셋으로 갈린다 |
| **공격** | J | 보스가 굳은 틈에 넣는다. **한 번이 0.50초**로 느리다 — 그림의 6프레임(12fps)을 끝까지 재생하는 길이이고, 칼은 5번째 장에서 나간다 |
...
③ 망 학습 (Python)     f(플레이어 잠재, 캐릭터, 패턴 태그) → 피격 확률
                       ...
                       **캐릭터 축은 지금 1종이다** — 상수라 아무것도 안 가르지만,
                       축을 지우면 캐릭터가 늘 때 데이터를 처음부터 다시 만들어야 한다
