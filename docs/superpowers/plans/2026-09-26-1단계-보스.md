...
스크린샷 대본의 이름만 따라간다(찍는 장면은 Task 7 이 다시 짠다):
...
Expected: `out/shots/battle-10e-boss-exhausted.png` 의 보스가 take-hit 마지막 장 · 푸른 톤이다. `docs/shots/` 는 그대로다.
...
Run: `HITBOXES=1 tools/build.sh shots`
Expected: `docs/shots/` 는 그대로다. `out/shots/` 에서 둘을 본다:
