...
**Tech Stack:** Godot 4.7.2 mono (forward_plus, 데스크톱 전용) · net8.0 · C# 12 · xUnit v3 + Shouldly · 데이터는 `prerelu/data/*.json`

**Spec:** [`docs/superpowers/specs/2026-09-22-전투코어-design.md`](../specs/2026-09-22-전투코어-design.md)

## Global Constraints

- **규칙 층은 Godot 을 모른다.** `prerelu/battle/rules/**` 에 `using Godot;` 를 쓰지 않는다. 새 순수 파일은 `tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj` 의 `<Compile Include>` 목록에 손으로 한 줄 넣는다 — 그 목록이 곧 "무엇이 규칙인가"의 선언이고, 커밋 훅의 TDD 게이트도 그것을 읽는다.
- **수치는 코드가 아니라 데이터에.** `prerelu/data/*.json` 이 진실이다. 예외는 `Arena.Gravity` 하나이고 그것도 이름 붙인 `const` 다.
...
- **네임스페이스:** 규칙 층 `PreReLU.Battle.Rules` · 씬 `PreReLU.Battle` · 뷰 `PreReLU.Battle.View` · 디버그 `PreReLU.Battle.Debug`.
...
```
prerelu/battle/rules/          ← 전부 순수 C#. Godot 을 모른다
...
tests/PreReLU.Rules.Tests/Battle/   각 규칙 파일의 짝 + TestConfigs.cs
...
**Files:**
- Create: `prerelu/battle/rules/InputFrame.cs`
- Create: `prerelu/battle/rules/Arena.cs`
- Create: `prerelu/battle/rules/FighterConfig.cs`
- Create: `prerelu/battle/rules/Fighter.cs`
- Create: `tests/PreReLU.Rules.Tests/Battle/TestConfigs.cs`
- Modify: `tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj`
- Test: `tests/PreReLU.Rules.Tests/Battle/FighterPhysicsTests.cs`
...
`tests/PreReLU.Rules.Tests/Battle/TestConfigs.cs`:
...
`tests/PreReLU.Rules.Tests/Battle/FighterPhysicsTests.cs`:
...
`prerelu/battle/rules/InputFrame.cs`:
...
`prerelu/battle/rules/FighterConfig.cs`:
...
`prerelu/battle/rules/Fighter.cs`:
...
`tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj` 의 core 링크 아래에 더한다:

```xml
    <!-- 전투 규칙. 이 목록이 곧 "무엇이 규칙인가"의 선언이다 — 커밋 훅의 TDD 게이트가 여기를 읽는다. -->
    <Compile Include="../../prerelu/battle/rules/InputFrame.cs" LinkBase="linked/battle/rules" />
    <Compile Include="../../prerelu/battle/rules/Arena.cs" LinkBase="linked/battle/rules" />
    <Compile Include="../../prerelu/battle/rules/FighterConfig.cs" LinkBase="linked/battle/rules" />
    <Compile Include="../../prerelu/battle/rules/Fighter.cs" LinkBase="linked/battle/rules" />
```
...
```bash
git add prerelu/battle tests/PreReLU.Rules.Tests
...
**Files:**
- Modify: `prerelu/battle/rules/Fighter.cs`
- Test: `tests/PreReLU.Rules.Tests/Battle/FighterActionTests.cs`
...
`tests/PreReLU.Rules.Tests/Battle/FighterActionTests.cs`:
...
**Files:**
- Create: `prerelu/battle/rules/HitBox.cs`
- Create: `prerelu/battle/rules/PatternDef.cs`
- Create: `prerelu/battle/rules/PatternRunner.cs`
- Create: `prerelu/data/patterns.json` (패턴 2개 — 여섯은 Task 7)
- Modify: `tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj`
- Test: `tests/PreReLU.Rules.Tests/Battle/PatternRunnerTests.cs`
- Test: `tests/PreReLU.Rules.Tests/Battle/PatternDataTests.cs`

**Interfaces:**
- Consumes: `PreReLU.Core.JsonData<T>` (베이스에 이미 있다)
- Produces:
...
`tests/PreReLU.Rules.Tests/Battle/PatternRunnerTests.cs`:
...
`tests/PreReLU.Rules.Tests/Battle/PatternDataTests.cs`:
...
`prerelu/battle/rules/HitBox.cs`:
...
`prerelu/battle/rules/PatternDef.cs`:
...
`prerelu/battle/rules/PatternRunner.cs`:
...
`prerelu/data/patterns.json` — 둘로 시작한다:
...
```xml
    <Compile Include="../../prerelu/battle/rules/HitBox.cs" LinkBase="linked/battle/rules" />
    <Compile Include="../../prerelu/battle/rules/PatternDef.cs" LinkBase="linked/battle/rules" />
    <Compile Include="../../prerelu/battle/rules/PatternRunner.cs" LinkBase="linked/battle/rules" />
```
...
**Files:**
- Create: `prerelu/battle/rules/HitResolver.cs`
- Modify: `tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj`
- Test: `tests/PreReLU.Rules.Tests/Battle/HitResolverTests.cs`
...
`tests/PreReLU.Rules.Tests/Battle/HitResolverTests.cs`:
...
`prerelu/battle/rules/HitResolver.cs`:
...
```xml
    <Compile Include="../../prerelu/battle/rules/HitResolver.cs" LinkBase="linked/battle/rules" />
```
...
**Files:**
- Create: `prerelu/battle/rules/Boss.cs`
- Create: `prerelu/battle/rules/BattleSim.cs`
- Create: `prerelu/data/fighters.json`
- Modify: `prerelu/core/Det.cs` (`Domain` 에 상수 추가)
- Modify: `tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj`
- Test: `tests/PreReLU.Rules.Tests/Battle/BattleSimTests.cs`
- Test: `tests/PreReLU.Rules.Tests/Battle/FighterDataTests.cs`

**Interfaces:**
- Consumes: Task 1~4 전부 + `PreReLU.Core.Det` · `PreReLU.Core.JsonData<T>`
- Produces:
...
`tests/PreReLU.Rules.Tests/Battle/BattleSimTests.cs`:
...
`tests/PreReLU.Rules.Tests/Battle/FighterDataTests.cs`:
...
`prerelu/core/Det.cs` 의 `Domain` 클래스 안, `Name` 메서드 **위에** 넣는다:
...
`prerelu/battle/rules/BattleSim.cs`:
...
```xml
    <Compile Include="../../prerelu/battle/rules/Boss.cs" LinkBase="linked/battle/rules" />
    <Compile Include="../../prerelu/battle/rules/BattleSim.cs" LinkBase="linked/battle/rules" />
```
...
**Files:**
- Create: `prerelu/battle/rules/DodgeEvent.cs`
- Create: `prerelu/battle/rules/PlayerAxes.cs`
- Modify: `prerelu/battle/rules/BattleSim.cs` (이벤트 기록)
- Modify: `tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj`
- Test: `tests/PreReLU.Rules.Tests/Battle/PlayerAxesTests.cs`
...
`tests/PreReLU.Rules.Tests/Battle/PlayerAxesTests.cs`:
...
`prerelu/battle/rules/DodgeEvent.cs`:
...
`prerelu/battle/rules/PlayerAxes.cs`:
...
```xml
    <Compile Include="../../prerelu/battle/rules/DodgeEvent.cs" LinkBase="linked/battle/rules" />
    <Compile Include="../../prerelu/battle/rules/PlayerAxes.cs" LinkBase="linked/battle/rules" />
```

- [ ] **Step 5: 전투가 정말 이벤트를 남기는지 본다**

`tests/PreReLU.Rules.Tests/Battle/BattleSimTests.cs` 에 더한다:
...
**Files:**
- Modify: `prerelu/data/patterns.json` (2개 → 6개)
- Create: `prerelu/battle/rules/BotPolicy.cs`
- Create: `prerelu/battle/debug/BattleDemo.cs`
- Modify: `prerelu/core/Game.cs` (`--battle-demo` 분기)
- Modify: `tools/build.sh` (`demo` 서브커맨드)
- Modify: `tests/PreReLU.Rules.Tests/PreReLU.Rules.Tests.csproj`
- Test: `tests/PreReLU.Rules.Tests/Battle/BotPolicyTests.cs`
...
`prerelu/data/patterns.json` 에 더한다 (기존 둘은 그대로):
...
`tests/PreReLU.Rules.Tests/Battle/BotPolicyTests.cs`:
...
`prerelu/battle/rules/BotPolicy.cs`:
...
`prerelu/core/Det.cs` 의 `Domain` 에 스트림 하나를 더한다 (`PatternPick` 아래):
...
```xml
    <Compile Include="../../prerelu/battle/rules/BotPolicy.cs" LinkBase="linked/battle/rules" />
```
...
`prerelu/battle/debug/BattleDemo.cs`:
...
namespace PreReLU.Battle.Debug;
...
`prerelu/core/Game.cs` 의 `_Ready` 안, `--tour` 분기 아래에 더한다:
...
`tests/PreReLU.Rules.Tests/Battle/ReplayGoldenTests.cs`:
...
- Create: `prerelu/assets/LICENSES.md`
- Create: `prerelu/assets/.gitkeep`
- Modify: `.gitignore`
- Modify: `CONTRIBUTING.md` (받는 법 한 절)

**Interfaces:**
- Consumes: 없음
- Produces: `prerelu/addons/duelyst_animated_sprites/` 아래에 SpriteFrames `.tres` 와 시트 `.png`. Task 9 의 뷰가 `res://addons/duelyst_animated_sprites/spriteframes/units/<id>.tres` 로 읽는다.
...
DEST = pathlib.Path(__file__).resolve().parent.parent / "prerelu" / "addons" / "duelyst_animated_sprites"
...
# `prerelu/addons/duelyst_animated_sprites/*` 로 쓰면 하위 폴더가 통째로 빠져 되살릴 수 없다. 그래서 네 줄이다.
prerelu/addons/duelyst_animated_sprites/**
!prerelu/addons/duelyst_animated_sprites/**/
!prerelu/addons/duelyst_animated_sprites/**/*.import
!prerelu/addons/duelyst_animated_sprites/**/*.tres
!prerelu/addons/duelyst_animated_sprites/duelyst-LICENSE
!prerelu/addons/duelyst_animated_sprites/duelyst-README.md
```

- [ ] **Step 3: 출처를 적는다**

`prerelu/assets/LICENSES.md`:
...
그리고 문서 어딘가에 한 줄: **에셋 원본은 저장소에 없다** — 받는 법과 출처는 [`prerelu/assets/LICENSES.md`](prerelu/assets/LICENSES.md).
...
**Files:**
- Create: `prerelu/battle/Battle.tscn` · `prerelu/battle/Battle.cs`
- Create: `prerelu/battle/view/FighterView.cs`
- Create: `prerelu/battle/view/BossView.cs`
- Create: `prerelu/battle/view/BattleHud.cs`
- Modify: `prerelu/core/Game.cs` (`Scene.Battle` 추가)
- Modify: `prerelu/play/Play.cs` (전투로 가는 버튼) 또는 `title/Title.cs`
...
`prerelu/battle/view/FighterView.cs`:

```csharp
using Godot;

namespace PreReLU.Battle.View;
...
`prerelu/battle/view/BossView.cs`:
...
`prerelu/battle/view/BattleHud.cs`:
...
`prerelu/battle/Battle.cs`:

```csharp
using System.Collections.Generic;
using Godot;
using PreReLU.Battle.Rules;
using PreReLU.Battle.View;
using PreReLU.Core;

namespace PreReLU.Battle;
...
`prerelu/project.godot` 에 절을 더한다:
...
`prerelu/battle/Battle.tscn` — 손으로 쓴다. 에디터를 띄울 필요가 없다.
...
`prerelu/play/Play.cs` 의 버튼이 전투로 가게 고친다:
