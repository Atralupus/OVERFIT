...
- Produces:
  ...
  - `TestConfigs.UntilWindup(BattleSim)` · `TestConfigs.UntilNear(BattleSim)` · `TestConfigs.UntilFired(BattleSim)` — 판정 시점을 러너와 같은 술어로 잰다
...
그리고 `TestConfigs.cs` 맨 위 `using` 에 `using Shouldly;` 를 더한다 (도우미가 단언을 쓴다).
...
**Interfaces:**
- Consumes: Task 1 의 `HitShape.Place` · Task 3 의 `ResolveLive` · `TestConfigs.SweepSim` · `TestConfigs.UntilWindup` · `TestConfigs.UntilFired` · Task 4 의 `PatternRunner.ShapeOf` · `Boss.Y` · `_attackShape`
- Produces:
