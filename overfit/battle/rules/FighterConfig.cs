...
public sealed class ComboStepDef
{
    ...
    /// 커밋한다: 행동 · 이동 · 점프 · 가드가 막히고, 그림은 시트를 끝까지 흘린 뒤 칼을 내린 선 자세에 멈춘다(마지막 장은 흩어지는
    /// 흰 궤적이라 붙들지 않는다 · <c>FighterView</c>). 이어지는 칼이 있으면 안 붙는다 — 2타는 지금처럼 1타가 끝나는 틱에 서고,
    /// 1타의 경직 중에 누른 J 도 그 틱에 2타가 된다. 한 번만 치고 마는 사람만 선다. 2타 뒤에는 이을 칼이 없어 언제나 붙는다.
    /// 유저: "1타공격 후 경직, 2타는 2타까지 공격후에는 좀더 오래 경직이 있게".
