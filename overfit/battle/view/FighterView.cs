...
public partial class FighterView : Node2D
{
    ...
    /// 주황으로 해 봤을 때는 가드가 깨질 때마다 같이 터지던 보스의 판정 충격파(호박색)와 같은 프레임에 겹쳐 둘이
    /// 한 덩어리가 됐다. 그 충격파는 걷었지만(#81 — 보스 공격에 동그라미를 안 그린다) 보라인 이유는 첫 문장이 그대로 선다.
    /// </para>
    /// </summary>
    ...
    /// 스태미나를 다 써 든 탈진과 같은 그림이고, 붕괴만 이 고리가 더 붙는다. 캐릭터의 링이라 남긴다(#81 · 유저 2026-09-26).
    /// </summary>
