...
#FFFFFF, 왕의 흰 털깃·수염은 한 픽셀도 빠짐없이 #F5EEEE(245, 238, 238) 다. 알파도 다섯 프레임 전부 0 · 255 뿐이다.
...
사각형이 궤적 말고 다른 그림(몸 · 옷 · 털)을 덮고 있으면 거기서 보인다. 그리는 사각형은 도구가 뽑은 값
(파일에 쓰는 그 값)을 월드 px 에서 region 픽셀로 **되돌린** 것이라, 좌표 변환이 틀려도 그림에서 어긋나 보인다.
"""
